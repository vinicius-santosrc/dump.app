...
export default function Account() {
    ...
    const [mentionsUser, setmentionsUser] = useState("")
        ...
        databases
            ...
            .then((res) => {
                setmentionsUser(
                    res.documents
                        .map((post) => {
                            if (post.mentions.includes(ID_ACCOUNT)) {
                                return (
                                    <article
                                        title={`${post.legenda}`}
                                        className="dump-user-account"
                                        id={post.$id}
                                        tabIndex={0}
                                        role="article"
                                        key={post.$id}
                                    >
                                        <Link to={window.location.origin + "/posts/" + post.$id}>
                                            <img
                                                className="dump-image-show"
                                                alt={post.legenda}
                                                id={`DPid_` + post.$id}
                                                src={post.filePost}
                                            />
    ...
    async function sendNotification(PHOTO_REL, USERPUBLIC, typeNotification) {
        try {
    ...
    async function followUser() {
        ...
        try {
            ...
            ButtonActionProfile()
            sendNotification('', ID_ACCOUNT, "follow")

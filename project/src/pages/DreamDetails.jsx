...
import { DreamsHeader } from "../components/pages/dreams/header";
...
import DreamItem from "../components/pages/dreams/Dream";
...
export default function DreamDetails() {
    ...
    const getAllDreams = async () => {
        const dreams = await databases.listDocuments(
        ...
        setDreams(dreams.documents)

    };

    useEffect(() => {
        getAllDreams()
        document.title = "Dreams - Dump"
    }, [])
    ...
    return (
        <>
            <DreamsHeader />
            <section
                aria-label="Timeline dos Dreams"
                tabindex="0"
                role="main"
                className="Dreams-Dump-Page dreamspage "
            >
                {Dreams.map((dream) => {
